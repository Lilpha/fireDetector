...
실시간 YOLO 기반 화재 및 동물 감지 시스템입니다. 백엔드(main.py)에서 YOLO 모델을 실행하고, 프론트엔드(Streamlit)에서 실시간 모니터링 대시보드를 제공합니다.
또한 Google Gemini AI를 활용하여 화재 상황을 심층 분석합니다.
...
### 🐾 동물 감지 (NEW)
- **YOLO v8 Nano 기반 동물 감지**: `yolov8n.pt` 모델을 사용하여 반려동물 및 사람 감지
- **성능 최적화**: 프레임 스킵(3프레임마다 감지)을 적용하여 실시간 성능 유지
- **감지 대상**: 개, 고양이, 새, 사람 등 주요 객체 인식
- **이벤트 로깅**: 동물 감지 이벤트를 `animal_events.json`에 기록

### 🤖 Gemini AI 화재 분석 (NEW)
- **Google Gemini 2.5 Flash 모델 연동**: 화재 감지 시 이미지를 분석하여 위험도 판단
- **위험 단계 분류**: 관심 / 경고 / 위험 / 대피 4단계로 상황 분석
- **상세 리포트**: 발화 위치, 원인 추정, 불길 강도 등을 100자 이내로 요약하여 대시보드에 표시
...
- Google Gemini API Key (AI 분석 기능 사용 시)
...
프로젝트 루트에 `.env` 파일을 생성하고 Google API Key를 입력하세요.

```env
GOOGLE_API_KEY=your_api_key_here
```

### 3️⃣ 백엔드 실행 (YOLO 감지 서버)
...
```
화재 감지 모델 클래스: {0: 'fire', ...}
동물 감지 모델 클래스: {0: 'person', 15: 'cat', 16: 'dog', ...}
성능 최적화: 매 3프레임마다 동물 감지
--- 실시간 화재 + 동물 감지를 시작합니다 ---
✓ 소켓 서버 대기 중: localhost:5005
```

### 4️⃣ 프론트엔드 실행 (Streamlit 대시보드)
